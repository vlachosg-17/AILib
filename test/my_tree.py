...
from utils.helpers import load_mushroom, load_iris, load_abalone, numerize_class, split_sets
from utils.metric import *
from ml.gini_index import Gini
from utils.helpers import overSample, vote
# from plot_tree import layout
from graph_tree.plot_tree import set_layout, axes_off
...
class Node:
    ...
    def best_split(self, features=None):
        ...
        self.gini, self.thres, self.var, (self.left_data, self.right_data) = \
             Gini(self.X, self.Y).optimal_split(features=features, state="list")
...
class SubTree:
    ...
    def __init__(self, tree, parent=None, depth=0, number=1):
        ...
        self.children = [SubTree(t, self, depth+1) for key, t in tree.items() if key is not "parent"]
...
class DTC:
    def __init__(self, max_depth = float("inf"), node_size=20, max_features=None, folds=None, label=1):#X, y
        ...
        if max_features is not None: # max_features = X.shape[1]
    ...
    def grow(self, node, store, time=None):
        if time is not None: time.update(1)
        ...
        if self.is_terminal(left_child):
            ...
            self.grow(left_child, store["left"], time=time)
        
        if self.is_terminal(right_child):
            right_child.label = self.terminal(right_child)
            store["right"] = right_child.dict
        else:
            self.grow(right_child, store["right"], time=time)
        return store
         
    def fit_(self, X, y):
        self.root = Node(X, y, depth=0)
        tree = {"parent": self.root}
        time=tqdm(total=self.max_num_nodes, ncols=50, leave=False)
        self.grow(self.root, tree, time=time)
        self.tree = SubTree(tree)
        return self.tree

    def fit(self, X, y, label=1):
        dt =  DTC(
    ...
    def iter_kfolds(self, X, y, k):
        X, y = shuffle(X, y)
        num_obs_fold = X.shape[0]//k
        for i in range(1, k):
            train_X, train_y = X[i*num_obs_fold:], y[i*num_obs_fold:]
            test_X, test_y = X[:i*num_obs_fold], y[:i*num_obs_fold]
            yield (train_X, train_y), (test_X, test_y)
    
    def cross_validation(self, X, y, k=10):
        acc = []
        for (trX, trY), (teX, teY) in self.iter_kfolds(X, y, k=k):
...
if __name__ == "__main__":
    ...
    # dt = DTC(trainX, trainY, max_depth=15, node_size=10, folds=10)
    ...
    # # evaluate the dicision with my tree
    # dt = DTC(trainX, trainY, max_depth=10, node_size=round(trainX.shape[0]/10), folds=10)
    ...
    #                 l="positive")
    # # evaluate the dicision with my tree
    # dt = DTC(trainX, trainY, max_depth=10, node_size=round(trainX.shape[0]/10))
    # predY = dt.predict(teX)
    # m = metrics(predY, teY, model=dt)
    # print_metrics(m)
    # # plot the decision my tree
