...
class CNL2D:
    ...
    def conv2d_channels_(self, X, K, mode="valid"):
        ...
        if mode=="valid":
            ...
            for c in range(Z.shape[0]):
                # for (xc, kc) in zip(X, K[c]):
                #     Z[c] += conv2d(xc, kc, xy_stride=self.xy_stride, tp=mode)
                # Z[c] = sum([conv2d(xc, kc, xy_stride=self.xy_stride, tp=mode) for (xc, kc) in zip(X, K[c])])
                Z[c] = sum([correlate2d(xc, kc, mode=mode) for (xc, kc) in zip(X, K[c])])
        if mode=="full":
    ...
    def conv2d_cartesian_(self, A, B):
        ...
        C0, C1 = A.shape[0], B.shape[0]
        # return np.array([[conv2d(A[c0], B[c1]) for c1 in range(C1)] for c0 in range(C0)])
        return np.array([[correlate2d(A[c0], B[c1], mode="valid") for c0 in range(C0)] for c1 in range(C1)])
    def forward(self, X):
        # Dx C0 x H x W
        self.x = X 
        # D x C0 x Hp x Wp
    ...
    def backward(self, node):
        ...
        self.DxE = np.array([self.conv2d_channels_(self.DzE[d], rot180(self.weights), mode="full") for d in range(self.x.shape[0])])
        return self.DxE
