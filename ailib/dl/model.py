    ...
    def init_params(self, load):
        if load == False:
            ...
            for l, layer in enumerate(self.Layers): 
                ...
                if layer.id == "CNL2D":
                    p = layer.C0 * layer.Hk * layer.Wk
                    init_w = np.random.uniform(-np.sqrt(1/p), np.sqrt(1/p), size=[layer.C1, layer.C0, layer.Hk, layer.Wk])
                    init_b = np.random.uniform(-np.sqrt(1/p), np.sqrt(1/p), size=layer.d_out)    
                    self.Layers[l].weights, self.Layers[l].bias = init_w, init_b
        if load == True:    
