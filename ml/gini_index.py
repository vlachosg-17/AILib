import numpy as np
from utils.helpers import load_iris, load_mushroom
...
class Gini:
    ...
    def powerset(self, s, remove=True):
        ...
        for i in range(1, 2**b+1):
            a.append([s[j] for j in range(b) if (i & (2**j))])
            
        if remove:
            if len(a[:-2]) == 0: return a[:-1]
            else: return a[:-2]
        else:
            return a
    ...
    def make_thresholds(self, x):
        ...
        if tp(x) in self.types:
            ...
            if len(v)==1: thrs=v
            else: thrs = np.array([(v[k-1]+v[k]) / 2 for k in range(1, len(v))])
        else: 
    ...
    def make_groups(self, x, thresh):
        ...
        if any([isinstance(thresh, tp) for tp in self.types]):
    ...
    def gini(self, D):
        ...
        return 1.0 - np.sum([(count/len(D))*(count/len(D)) for count in counts])
    ...
    def optimal_split(self, state="dict", features=None):
        ...
        for i , attr in enumerate(self.X.T):
